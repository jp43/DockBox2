...
class BinaryFocalCrossentropy(tf.keras.losses.Loss):
    ...
    def __init__(self, apply_class_balancing=False, alpha=0.25, gamma=2.0, from_logits=False, label_smoothing=0.0):
    ...
    def call(self, labels, preds):
        ...
        p_t = tf.where(tf.equal(labels, 1), preds_clipped, 1 - preds_clipped)

        if self.apply_class_balancing:
            alpha_factor = tf.ones_like(labels_f) * self.alpha 
            alpha_t = tf.where(tf.equal(labels, 1), alpha_factor, 1 - alpha_factor)
        else:
